...
$ProjectRoot = $null
if (Test-Path -LiteralPath $FixedProject -PathType Container) {
    $ProjectRoot = $FixedProject
}
elseif ($PSScriptRoot -and (Test-Path -LiteralPath (Join-Path $PSScriptRoot "01-FUENTE") -PathType Container)) {
...
New-Item -ItemType File -Force -LiteralPath $Log | Out-Null
$script:Log = $Log
...
try {
    ...
    Add-Content -LiteralPath $script:Log -Value ("[" + (Get-Date -Format "yyyy-MM-dd HH:mm:ss.fff") + "] TRANSCRIPT_START=FAIL;ERROR=" + $_.Exception.Message) -Encoding UTF8
}
...
function New-OverlayJar {
    ...
    Copy-Item -LiteralPath $ActivePath -Destination $TempPath -Force
...
function Test-ValidSource {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $Candidate "pom.xml"))) {
        return $false
    }
...
function Find-Java17 {
    ...
    if (Test-Path -LiteralPath $javaParent -PathType Container) {
        $homes += @(Get-ChildItem -LiteralPath $javaParent -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "jdk-17*" } | ForEach-Object { $_.FullName })
    }
...
function Find-Maven {
    ...
    New-Item -ItemType Directory -Force -LiteralPath $toolsDir | Out-Null
    ...
    $expected = [regex]::Match((Get-Content -LiteralPath $checksumFile -Raw), "[0-9a-fA-F]{128}").Value.ToUpperInvariant()
    if ($expected.Length -ne 128) {
        throw "No se pudo leer SHA-512 de Apache Maven."
    }
    $actual = (Get-FileHash -LiteralPath $archive -Algorithm SHA512).Hash.ToUpperInvariant()
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $mavenDir "bin\mvn.cmd") -PathType Leaf)) {
...
try {
    ...
    elseif (Test-Path -LiteralPath $cloneCandidate) {
        ...
        New-Item -ItemType Directory -Force -LiteralPath (Split-Path -Parent $cloneCandidate) | Out-Null
    ...
    foreach ($check in $markerChecks) {
        ...
        if (-not (Select-String -LiteralPath $full -Pattern $check.Pattern -SimpleMatch -Quiet)) {
    ...
    foreach ($relative in $badFiles) {
        $full = Join-Path $Source ($relative -replace "/", "\")
        if (Select-String -LiteralPath $full -Pattern "void var22_", "for (void " -SimpleMatch -Quiet) {
    ...
    if (-not $Activate) {
        ...
        $processes = @(Get-CimInstance -ClassName Win32_Process -ErrorAction SilentlyContinue | Where-Object {
            ...
            $exe.StartsWith($Active, [StringComparison]::OrdinalIgnoreCase) -or ($cmd.IndexOf($Active, [StringComparison]::OrdinalIgnoreCase) -ge 0)
        })
        ...
        $activeClientMatches = @()
        if (Test-Path -LiteralPath (Join-Path $Active "client\lib") -PathType Container) {
            $activeClientMatches += @(Get-ChildItem -LiteralPath (Join-Path $Active "client\lib") -Filter "mage-client-1.4.61.jar" -File -ErrorAction SilentlyContinue)
        }
        if ($activeClientMatches.Count -eq 0) {
            $activeClientMatches += @(Get-ChildItem -LiteralPath $Active -Filter "mage-client-1.4.61.jar" -File -Recurse -ErrorAction SilentlyContinue)
        }
        ...
        foreach ($libraryDir in @((Join-Path $Active "client\lib"), (Join-Path $Active "server\lib"))) {
            if (Test-Path -LiteralPath $libraryDir -PathType Container) {
                $activeCommonMatches += @(Get-ChildItem -LiteralPath $libraryDir -Filter "mage-common-1.4.61.jar" -File -ErrorAction SilentlyContinue)
            }
        }
        ...
        if ($Launch) {
            ...
            $serverLauncher = @($serverLaunchers | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf } | Select-Object -First 1)
            if ($serverLauncher.Count -eq 0) {
                $serverLauncher = @(Get-ChildItem -LiteralPath $Active -Filter "*.bat" -File -Recurse -ErrorAction SilentlyContinue |
                    Where-Object { Select-String -LiteralPath $_.FullName -Pattern "mage.server.Main" -SimpleMatch -Quiet -ErrorAction SilentlyContinue } |
            ...
            $clientLauncher = @($clientLaunchers | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf } | Select-Object -First 1)
            if ($clientLauncher.Count -eq 0) {
                $clientLauncher = @(Get-ChildItem -LiteralPath $Active -Filter "*.bat" -File -Recurse -ErrorAction SilentlyContinue |
                    Where-Object { Select-String -LiteralPath $_.FullName -Pattern "mage.client.MageFrame", "mage.client.Main" -SimpleMatch -Quiet -ErrorAction SilentlyContinue } |
                    Select-Object -First 1 | ForEach-Object { $_.FullName })
            }
            if ($clientLauncher.Count -eq 0) {
    ...
    if ($Activate -and ($BackupItems.Count -gt 0)) {
        ...
        foreach ($backup in $BackupItems) {
            try {
                Copy-Item -LiteralPath $backup.Backup -Destination $backup.Active -Force
    ...
    foreach ($plan in $OverlayPlans) {
        if ((Test-Path -LiteralPath $plan.Temp -PathType Leaf) -and ($ExitCode -eq 0)) {
            Remove-Item -LiteralPath $plan.Temp -Force
            Write-Log "TEMP_CLEAN=PASS;PATH=$($plan.Temp)"
        }
        elseif (Test-Path -LiteralPath $plan.Temp -PathType Leaf) {
    ...
    if ($TranscriptStarted) {
        try {
            ...
            if (Test-Path -LiteralPath $TranscriptLog -PathType Leaf) {
                Add-Content -LiteralPath $script:Log -Value ("[" + (Get-Date -Format "yyyy-MM-dd HH:mm:ss.fff") + "] TRANSCRIPT_FINAL=PASS;PATH=" + $TranscriptLog + ";BYTES=" + (Get-Item -LiteralPath $TranscriptLog).Length + ";SHA256=" + (Get-Hash $TranscriptLog)) -Encoding UTF8
            }
        }
        catch {
            Add-Content -LiteralPath $script:Log -Value ("[" + (Get-Date -Format "yyyy-MM-dd HH:mm:ss.fff") + "] TRANSCRIPT_FINAL=FAIL;ERROR=" + $_.Exception.Message) -Encoding UTF8
        }
    }
}
