...
$ExitCode = 1
$script:StartedLauncherPids = @()
$script:LaunchCleanupArmed = $false
$BackupRoot = $null
$BackupItems = @()
$OverlayPlans = @()
$ReplacedPlans = @()
...
function Start-ExistingLauncher {
    ...
    $runtimeHome = $null
    if ($RuntimeJava) {
        if (-not (Test-Path -Path $RuntimeJava -PathType Leaf)) {
            throw "Java de runtime no existe: $RuntimeJava"
        }
        $runtimeBin = Split-Path -Parent $RuntimeJava
        $runtimeHome = Split-Path -Parent $runtimeBin
        Write-Log ("LAUNCH_" + $Label + "_JAVA_OVERRIDE=JAVA_HOME:" + $runtimeHome + ";PATH_FIRST:" + $runtimeBin)
    }
    if ($Path.EndsWith(".ps1", [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        if ($RuntimeJava) {
            $command = 'set "JAVA_HOME=' + $runtimeHome + '"&&set "PATH=' + $runtimeBin + ';%PATH%"&&call "' + $Path + '"'
        }
        else {
            $command = 'call "' + $Path + '"'
        }
        $process = Start-Process -FilePath $cmd -ArgumentList @("/d", "/s", "/c", $command) -WorkingDirectory (Split-Path -Parent $Path) -PassThru
    }
...
try {
    ...
    if (-not $Activate) {
        ...
        if ($Launch) {
            ...
            $activeJavaHome = Split-Path -Parent $activeJavaBin
            $env:JAVA_HOME = $activeJavaHome
            $env:Path = $activeJavaBin + ";" + [string]$env:Path
            Write-Log "JAVA_HOME_RUNTIME=$env:JAVA_HOME"
            Write-Log "JAVA_RUNTIME=$activeJava8"
            Write-Log "JAVA_PATH_RUNTIME_FIRST=$activeJavaBin"
            $resolvedRuntimeJava = (Get-Command java.exe -ErrorAction Stop).Source
            if ([IO.Path]::GetFullPath($resolvedRuntimeJava) -ine [IO.Path]::GetFullPath($activeJava8)) {
                throw "PATH de runtime no resuelve Java 8u201. Resuelto=$resolvedRuntimeJava;Esperado=$activeJava8"
            }
            Write-Log "JAVA_COMMAND_RUNTIME=$resolvedRuntimeJava;EXPECTED=$activeJava8;VERIFY=PASS"
            $runtimeVersionOutput = Invoke-NativeLogged -FilePath $resolvedRuntimeJava -Arguments @("-version") -Label "JAVA_RUNTIME_PATH_VERSION"
            $runtimeVersion = (@($runtimeVersionOutput) -join " ")
            if ($runtimeVersion -notmatch 'version "1\.8\.') {
                throw "El java.exe efectivo no es Java 8: $runtimeVersion"
            }
            Write-Log "JAVA_RUNTIME_VERSION=PASS"
            ...
            $serverProcess = Start-ExistingLauncher -Path $serverPath -Label "SERVER" -RuntimeJava $activeJava8
            $script:StartedLauncherPids += [int]$serverProcess.Id
            $script:LaunchCleanupArmed = $true
            ...
            $clientProcess = Start-ExistingLauncher -Path $clientPath -Label "CLIENT" -RuntimeJava $activeJava8
            $script:StartedLauncherPids += [int]$clientProcess.Id
    ...
    if ($script:LaunchCleanupArmed -and ($script:StartedLauncherPids.Count -gt 0)) {
        Write-Log "LAUNCH_ROLLBACK_SCAN=ARMED;TRACKED_PIDS=$($script:StartedLauncherPids -join ',')"
        $trackedIds = @($script:StartedLauncherPids)
