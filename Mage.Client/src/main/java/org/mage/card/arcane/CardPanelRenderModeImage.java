...
public class CardPanelRenderModeImage extends CardPanel {
    ...
    public void doLayout() {
        ...
        if (triggerIndicatorPanel != null) {
            triggerIndicatorPanel.setLocation(realCardSize.x, realCardSize.y);
            triggerIndicatorPanel.setSize(realCardSize.width, realCardSize.height);
        }
    ...
    private static BufferedImage createTriggerIndicator() {
        ...
        try {
            ...
            g.drawString("T", (25 - fm.stringWidth("T")) / 2,
    ...
    private void updateTriggerIndicator(CardView card) {
        boolean active = card instanceof PermanentView
                && ((PermanentView) card).hasActiveTrigger();
        if (active && triggerIndicatorPanel == null) {
            triggerIndicatorPanel = new JPanel(null);
            triggerIndicatorPanel.setOpaque(false);
            triggerIndicatorButton = new JButton(new ImageIcon(createTriggerIndicator()));
            triggerIndicatorButton.setBorder(BorderFactory.createEmptyBorder());
            triggerIndicatorButton.setContentAreaFilled(false);
            triggerIndicatorButton.setFocusPainted(false);
            triggerIndicatorButton.setToolTipText("Triggered ability waiting on the stack");
            triggerIndicatorPanel.add(triggerIndicatorButton);
            add(triggerIndicatorPanel);
        }
        if (triggerIndicatorPanel != null) {
            triggerIndicatorPanel.setVisible(active && isPermanent());
            if (active) {
                triggerIndicatorButton.setBounds(2, 2, 25, 25);
            }
        }
    }
